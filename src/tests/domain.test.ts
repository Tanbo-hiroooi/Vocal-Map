...
describe('初期データと移行', () => {
  ...
  test('schema 6のデータを文字間配置対応のschemaへ移行する', () => { const current = { schemaVersion: 6, songs: [], symbols: createPresetSymbols(), settings: { defaultFontSize: 18, practiceFontSize: 28, lyricLineSpacing: 6, colorScheme: 'system' as const } }; expect(migrateData(current).schemaVersion).toBe(7); });
