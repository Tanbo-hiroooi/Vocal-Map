...
export function SongForm({song,onSave}:{song?:Song;onSave(song:Song):Promise<void>}) {
  ...
  return <View testID="song-form" style={styles.container}>
    <ScrollView testID="song-form-scroll" style={styles.scroll} contentContainerStyle={styles.form} keyboardShouldPersistTaps="handled"
      keyboardDismissMode={Platform.OS === 'ios' ? 'interactive' : 'on-drag'} automaticallyAdjustKeyboardInsets>
      <View><Text style={styles.kicker}>VOCAL MAP</Text><Text style={styles.title}>{song?'曲と歌詞を編集':'歌唱設計図を作る'}</Text><Text style={styles.lead}>歌詞は端末内だけに保存され、外部へ送信されません。</Text></View>
    ...
    <View testID="song-form-actions" style={styles.actions}>
      {!!message&&<Text accessibilityRole="alert" style={styles.error}>{message}</Text>}
      <Button label="保存する" onPress={()=>void submit()} loading={saving}/>
    </View>
  </View>;
}
const styles=StyleSheet.create({container:{flex:1,minHeight:0},scroll:{flex:1,minHeight:0},form:{gap:18,paddingBottom:18},actions:{gap:8,paddingTop:12,borderTopWidth:1,borderTopColor:colors.border,backgroundColor:colors.background},kicker:{fontSize:12,letterSpacing:2,color:colors.primary,fontWeight:'800'},title:{fontSize:28,fontWeight:'900',color:colors.text,marginTop:5},lead:{fontSize:14,color:colors.muted,lineHeight:21,marginTop:6},lyrics:{minHeight:240},error:{color:colors.danger}});
