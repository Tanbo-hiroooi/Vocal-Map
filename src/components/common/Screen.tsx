...
export function Screen({children, scroll=true, contentStyle}: PropsWithChildren<{scroll?:boolean;contentStyle?:ViewStyle}>) {
  ...
  const body=<View style={[styles.content,width < layout.compactWidth && styles.compact,!scroll && styles.fill,contentStyle]}>{children}</View>;
...
const styles=StyleSheet.create({safe:{flex:1,minHeight:0,backgroundColor:colors.background},scroll:{flexGrow:1},content:{width:'100%',maxWidth:layout.maxWidth,alignSelf:'center',padding:20,gap:16},fill:{flex:1,minHeight:0},compact:{padding:12,gap:12}});
